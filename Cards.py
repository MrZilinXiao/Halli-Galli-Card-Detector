...
def find_fruits_in_each_card(qCard, train_fruits_images): # 0~3 ['yellow', 'red', 'green', 'purple']
    ...
    purpleCount = 0
    for i in range(0,3):
        w, h = train_fruits_images[i].img.shape[::-1]
        res = cv2.matchTemplate(img, train_fruits_images[i].img, cv2.TM_CCOEFF_NORMED)
        locs = np.where(res >= 0.95)  # 准确度
        '''if i == 0:
            yellowCount = len(*locs[::-1])
        elif i == 1:
            redCount = len(*locs[::-1])
        elif i == 2:
            greenCount = len(*locs[::-1])
        else:
            purpleCount =len(*locs[::-1])
            # 数水果
        '''
        qCard.count[i]=len(*locs[::-1])
        for loc in zip(*locs[::-1]):
            qCard.warp = cv2.rectangle(img, loc, (loc[0] + w, loc[1] + h), (0, 0, 255), 3)
...
def draw_results(image, qCard):
    ...
    for i in range(0,3):
        ...
        if i == 0:
            ...
            fruit_kind = 'purple'
        cv2.putText(image, (fruit_counts + ' of'), (x - 60, y - 10), font, 1, (0, 0, 0), 3, cv2.LINE_AA)
        cv2.putText(image, (fruit_counts + ' of'), (x - 60, y - 10), font, 1, (50, 200, 200), 2, cv2.LINE_AA)
